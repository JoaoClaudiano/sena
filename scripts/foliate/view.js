...
// Load EPUB from URL using HTTP Range Requests — only fetches the entries needed,
...
export const makeBook = async file => {
    // For URL strings, use HTTP Range Requests so only requested ZIP entries are
    // transferred — the entire file is never downloaded all at once.
    if (typeof file === 'string') {
        try {
            const loader = await makeHttpLoader(file)
            const { EPUB } = await import('./epub.js')
            return new EPUB(loader).init()
        } catch (e) {
            // Range requests may be unsupported by the server; fall back to a
            // full download so the reader still works in that case.
            console.warn('HttpRangeReader failed, falling back to full download:', e)
